...
def humanize(req: HumanizeRequest, user: sqlite3.Row = Depends(get_current_user)):
    ...
    # Diagnosis-and-fix loop: identify remaining AI patterns and surgically
    # fix only those, rather than doing a full rewrite that might reintroduce
    # other patterns. Cap at 2 rounds to stay within Groq's rate limits.
    for round_num in range(2):
        patterns = diagnose_ai_patterns(text)
        logger.info("Round %d AI patterns detected: %s", round_num + 1, patterns)
        if not patterns:
            break
